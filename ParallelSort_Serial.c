...
{
    long num1 = *((long *)ptr2num1);
    long num2 = *((long *)ptr2num2);
...
long *merge(long *left, long *right, int l_end, int r_end)
{
    ...
    long *temp = malloc(sizeof(long) * l_end);
    ...
    if (n > 1)
    {
        ...
        else
        {
            ...
            for (int thread_num = 0; thread_num < p; thread_num++)
            {
                ...
                loc_a = malloc(loc_size * sizeof(long));
                memcpy(loc_a, a + start, loc_size * sizeof(long));
            ...
            for (int thread_num = 0; thread_num < p; thread_num++)
            {
                ...
                for (i = 0, j = 0; i < p; i++)
                {
                    ...
                    if (partition_size > 0)
                    {
                        memcpy(this_result + j, &(loc_a_ptrs[i][low]), partition_size * sizeof(long));
                        j += partition_size;
                    }
                }
...
{
    qsort(a, len, sizeof(long), lcompare);
}